    ...
    def get_file_items(self, *args) -> List[Nautilus.MenuItem]:
        ...
        for file in files:
            ...
            if file_mime in READ_FORMATS_IMAGE:
                return self.__nautilus_submenu_builder(WRITE_FORMATS_IMAGE,
                                              callback=convert_image,
                                              files=files)
            if file_mime in READ_FORMATS_AUDIO:
                return self.__nautilus_submenu_builder(WRITE_FORMATS_AUDIO,
                                              callback=convert_audio,
                                              files=files)
            if file_mime in READ_FORMATS_VIDEO:
                return self.__nautilus_submenu_builder(WRITE_FORMATS_VIDEO,
                                              callback=convert_audio,
                                              files=files)

# --- Build the context menu and submenus ---
    def __nautilus_submenu_builder(self, formats, callback, files):
